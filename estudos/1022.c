...
int main() {
    ...
    while (n--) {
        int numero1, den1, numero2, den2;
        char op;

        scanf("%d / %d %c %d / %d", &numero1, &den1, &op, &numero2, &den2);

        int res_num, res_den;

        switch (op) {
            case '+':
                res_num = numero1 * den2 + numero2 * den1;
                res_den = den1 * den2;
                break;
            case '-':
                res_num = numero1 * den2 - numero2 * den1;
                ...
                break;
            case '*':
                res_num = numero1 * numero2;
                res_den = den1 * den2;
                break;
            case '/':
                res_num = numero1 * den2;
                res_den = numero2 * den1;
                break;
        }
